...
features_combinations = {
    {"echo_doppler","delay", "movement_type"},
...
for bat_num = bat_nums
    ...
    for ith_window = 1:length(n_windows)
        for jj = 1:length(features_combinations)
            ...
            X_table.bat_num = string(ones(height(X_table), 1)*bat_num);
            X_table = X_table(X_table.("movement_type-1") == "woods" | X_table.("movement_type-1") == "field",:);
            all_data = [all_data;X_table];
        end
    end
end


[mdl_woods, mdl_field] = GLMEPredict(all_data, prediction_name);
mdls = mdl_woods;
cats = mdl_field;
...
for i = 1:length(mdls)
    eval(sprintf("mdl_%s = mdls{i}", cats{i}))
    save(base_fig_dir+"mdl_"+cats{i}, "mdl_"+cats{i})
end
close
